...
fn profile_picture_source() -> String {
  glevatar.new("heesungj7@gmail.com")
...
fn homepage_header() -> Element(a) {
  let subtitle = h2([id("homepage-subtitle")], [text("Hi 👋")])

  let title =
    h1([id("homepage-title")], [
      text("I'm "),
      span([class("p-given-name")], [text("Joseph")]),
    ])

  let github_link =
    a([href("https://github.com/heesungjang"), rel("me"), class("u-url")], [
      text("GitHub"),
    ])
  ...
  let linkedin_link =
    a(
      [
        href("https://www.linkedin.com/in/joseph-jang-4975a5239/"),
        rel("me"),
        class("u-url"),
      ],
      [text("LinkedIn")],
  ...
  let description =
    p([id("homepage-description"), class("p-note")], [
      text(
        "I love working with people, inspiring others, and being inspired by others.",
      ),
      br([]),
...
fn default_head(page_title: String, description: String) -> Element(a) {
  head([], [
    ...
    link([
      ...
      attribute.title("josephjang.me posts feed"),
      href("https://josephjang.me/feed.xml"),
    ]),
    meta([property("og:site_name"), content("Joseph Jang's blog")]),
    ...
    meta([property("twitter:creator"), content("@josephjang")]),
    link([
      rel("payload"),
